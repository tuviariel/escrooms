...
export const QuizData: React.FC<Partial<QuizDataProps>> = (props) => {
    ...
    const colorChange = () => {
        ...
        updatedColorOrder[quizDataPageNumber] =
            data?.category && data.category?.length - 1 > updatedColorOrder[quizDataPageNumber]
                ? updatedColorOrder[quizDataPageNumber] + 1
                : 0;
        setColorOrder(updatedColorOrder);
    };
    const checkOrderAnswer = () => {
        // console.log(colorOrder, data?.orderAnswer);
        let count = 0,
            i;
            ...
            {data && data.quizData.length > 1 ? (
                <div className="relative">
                    ...
                    <img
                        ...
                        alt="riddle - error..."
                        className={`${
                            data.type === "colorChange" ? "cursor-pointer h-auto" : "h-screen"
                        } w-full pt-10`}
                        onClick={() => data.type === "colorChange" && colorChange()}
                    />
                    {data.type === "colorChange" && (
                        <>
                            {colorOrder[quizDataPageNumber] !== -1 && (
                                <img
                                    src={
                                        data.category &&
                                        data.category[colorOrder[quizDataPageNumber]]
                                    }
                                    alt={`option #` + colorOrder[quizDataPageNumber] + 1}
