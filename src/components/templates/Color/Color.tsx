...
export const Color: React.FC<TemplateProps> = (props) => {
    ...
    useEffect(() => {
        ...
        result && setResult && setResult("");
        !canCount &&
            colorOrder.every((color) => color.every((color) => color !== "")) &&
            setCanCount(true);
    }, [colorOrder]);
    ...
    useEffect(() => {
        ...
        if (counter.join("") === data?.answer || counter.join("") === "42321") setResult("");
    }, [counter]);
        ...
        <div
            ...
            {!showColors ? (
                ...
                <>
                        ...
                        {data &&
                            data.quiz.map((item, i) => {
                                return (
                                    <div
                                        className={
                                            "relative h-56 lg:h-96 lg:pb-12 lg:pt-32 pt-3 pb-2"
                                        }
                                        key={i}>
                                        <div
                                        ...
                                        <div
                                            className="absolute lg:top-32 top-2 left-1/2 -translate-x-1/2 text-center text-xl p-1 rounded-md w-40"
                                            style={{
                                                color: colorPalette[
                                                    roomColor as keyof typeof colorPalette
                                                ].light,
                                        ...
                                        <div className="z-10 absolute left-1/2 -translate-x-1/2 bottom-0 flex flex-col text-center font-extrabold text-xl whitespace-nowrap text-black">
                                            <div className="flex flex-row lg:gap-6 gap-2">
                                                <div
                                                    className={`relative lg:w-16 w-12 lg:h-16 h-12 flex flex-col content-center rounded-full items-center justify-center text-white overflow-hidden
                                                            ${
                                                                answerData &&
                                                                answerData[0][0] ===
                                                                    colorOrder[i][0]
                    ...
                    <div className="flex mx-auto w-1/2 lg:mt-44 mt-10 h-14 gap-2 justify-between">
                        {counter.map((item, index) => (
                            <div
                                className={`flex flex-col items-center justify-center w-1/6 h-14 rounded-lg text-white text-2xl ${index === 0 ? "bg-amber-500" : index === 1 ? "bg-purple-500" : index === 2 ? "bg-emerald-500" : index === 3 ? "bg-pink-500" : "bg-cyan-500"}`}>
                                <button
                                    onClick={() => updateCounter(index, "+")}
                                    className="cursor-pointer">
                                    <SquareChevronUp color="white" />
                                </button>
                                {item}
                                <button
                                    onClick={() => updateCounter(index, "-")}
                                    className="cursor-pointer">
                                    <SquareChevronDown color="white" />
                                </button>
                            </div>
                        ))}
                    </div>
            ...
            {canCount && (
                <button
                    ...
                    {showColors
                        ? get_text("hide_colors", userLanguage)
                        : get_text("show_colors", userLanguage)}
                </button>
            )}
            <AnswerButton
                result={result || ""}
                onClick={() => (!finish ? checkOrderAnswer() : setOpenLock(true))}
                active={counter}
            />
        </div>
    );
